...
["men", "women", "courtTotal", "shuttleCount", "pricePerShuttle"].forEach((id) => {
...
function findOptimalSplit(total, men, women) {
  const targetDiff = 5000; // mặc định 5k như yêu cầu
  // Tính perWoman cơ bản
  ...
  // Đảm bảo chênh lệch chính xác >= targetDiff (vì làm tròn có thể làm sai)
  if (perMan - perWoman < targetDiff) {
    perMan = perWoman + targetDiff;
    perMan = roundUpToThousand(perMan);
  }

  // Nếu tổng thực tế < tổng dự kiến -> tăng từng người theo nghìn để ít hơn nhất
  let actualTotal = perMan * men + perWoman * women;
  while (actualTotal < total) {
    // ưu tiên tăng perWoman trước để giữ chênh lệch
    perWoman += 1000;
    // điều chỉnh perMan để giữ chênh lệch = targetDiff
    perMan = perWoman + targetDiff;
    perWoman = roundUpToThousand(perWoman);
    perMan = roundUpToThousand(perMan);
    actualTotal = perMan * men + perWoman * women;
    // Nếu vòng lặp vô hạn, bẻ ra
    if (perWoman > total + 1000000) break;
  }

  // đảm bảo chênh lệch là targetDiff (sau làm tròn)
...
function addMemberRow(init = {}) {
  ...
  if (init.gender) row.querySelector('[data-field="gender"]').value = init.gender;
  if (init.minutes !== undefined) row.querySelector('[data-field="minutes"]').value = init.minutes;
}
...
document.getElementById("calculateBtn").addEventListener("click", function () {
  ...
  // Nếu chỉ nam hoặc chỉ nữ -> chia đều
  let perMan = 0,
    perWoman = 0,
    actualTotal = 0;
  ...
  // Lấy danh sách thành viên để áp giảm cá nhân
  const memberRows = Array.from(memberContainer.querySelectorAll(".member-row"));
  ...
  // Tính tổng sau khi áp giảm cá nhân:
  // - Những thành viên không có dòng sẽ đóng theo perMan/perWoman
  let menRemaining = men;
  let womenRemaining = women;
  let adjustedTotal = 0;
  const memberResults = [];

  // Áp dụng cho thành viên có khai báo
  members.forEach((m) => {
    let base = m.gender === "man" ? perMan : perWoman;
    // số lần giảm 30 phút
    const steps = Math.floor(m.minutes / 30);
    const reduction = steps * 5000; // 5k/30 phút
    const adjusted = Math.max(0, base - reduction);
    memberResults.push({ name: m.name, gender: m.gender, base, minutes: m.minutes, reduction, adjusted });
    adjustedTotal += adjusted;
    if (m.gender === "man") menRemaining--;
    else womenRemaining--;
  });

  // Các thành viên còn lại (không khai báo) chịu base full
  ...
  html += `• Tiền cầu: ${scount} × ${pShut.toLocaleString("vi-VN")}đ = ${shuttleCost.toLocaleString("vi-VN")} VND<br><br>`;
  html += `<b>Tổng dự kiến:</b> ${totalProjected.toLocaleString("vi-VN")} VND<br>`;
  html += `<b>Tổng chia theo Nam/Nữ (sau làm tròn):</b> ${actualTotal.toLocaleString("vi-VN")} VND<br>`;
  html += `<b>Tiền mỗi Nam (base):</b> ${perMan.toLocaleString("vi-VN")} đ (x${men})<br>`;
  html += `<b>Tiền mỗi Nữ (base):</b> ${perWoman.toLocaleString("vi-VN")} đ (x${women})<br>`;
  html += `<b>Chênh lệch Nam/Nữ (base):</b> ${(perMan - perWoman).toLocaleString("vi-VN")} đ<br><br>`;

  if (memberResults.length > 0) {
    html += `<b>Điều chỉnh cá nhân (giảm 5k/30 phút):</b><br>`;
    memberResults.forEach((m) => {
      html += `• ${m.name || (m.gender === "man" ? "Nam" : "Nữ")} (${m.gender === "man" ? "Nam" : "Nữ"}): base ${m.base.toLocaleString("vi-VN")}đ, -${m.reduction.toLocaleString("vi-VN")}đ (tương đương ${m.minutes} phút) → ${m.adjusted.toLocaleString("vi-VN")}đ<br>`;
    });
    html += `<br>`;
  }

  html += `<b>Tổng thực thu sau điều chỉnh cá nhân:</b> ${adjustedTotal.toLocaleString("vi-VN")} VND<br>`;
  const diff = adjustedTotal - totalProjected;
  const diffLabel = diff >= 0 ? `+${diff.toLocaleString("vi-VN")}` : `${diff.toLocaleString("vi-VN")}`;
  html += `<b>Chênh lệch so với dự kiến:</b> ${diffLabel} VND<br>`;

  // Nếu muốn hiển thị danh sách chi tiết mỗi thành viên tổng hợp (không khai báo): hoán vị
  html += `<hr style="border:none;height:8px">`;
  html += `<b>Danh sách thanh toán (tổng từng người):</b><br>`;
  // Những khai báo trước
  memberResults.forEach((m, idx) => {
    html += `• ${m.name || ("Thành viên " + (idx + 1))}: ${m.adjusted.toLocaleString("vi-VN")}đ<br>`;
  });
  // Các thành viên còn lại (chưa khai báo)
  if (menRemaining > 0) {
    for (let i = 0; i < menRemaining; i++) {
      html += `• Nam (không khai báo): ${perMan.toLocaleString("vi-VN")}đ<br>`;
    }
  }
  if (womenRemaining > 0) {
    for (let i = 0; i < womenRemaining; i++) {
      html += `• Nữ (không khai báo): ${perWoman.toLocaleString("vi-VN")}đ<br>`;
    }
  }
...
// ====== LƯU Ý: Người dùng có thể thêm nhiều member, nếu không thêm thì áp base cho tất cả ======
// Example: để thử nhanh bạn có thể thêm vài thành viên bằng JS:
// addMemberRow({name:"A", gender:"man", minutes:30});
// addMemberRow({name:"B", gender:"woman", minutes:0});