...
document.getElementById("calculateBtn").addEventListener("click", function () {
  ...
  html += `<b>Tổng chia theo Nam/Nữ (sau làm tròn):</b> ${actualTotal.toLocaleString(
    "vi-VN"
  )} VND<br>`;
  ...
  if (memberResults.length > 0) {
    ...
    memberResults.forEach((m) => {
      html += `• ${m.name || (m.gender === "man" ? "Nam" : "Nữ")} (${
        m.gender === "man" ? "Nam" : "Nữ"
      }): base ${m.base.toLocaleString(
        "vi-VN"
      )}đ, -${m.reduction.toLocaleString("vi-VN")}đ (tương đương ${
        m.minutes
  ...
  memberResults.forEach((m, idx) => {
    ...
    }: ${m.adjusted.toLocaleString("vi-VN")}đ<br>`;
  });
